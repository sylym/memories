<template>
  <div style="display: flex;flex-direction: column;width: 100%;align-items: center;min-width: 1400px;" v-loading="loading">
    ...
    <div id="uploadContent" style="display: flex;width: 100%;min-width: 1400px;flex-direction: column;align-items: center">
      <div v-if="contentIndex === 0" style="width: 100%">
        <div id="work-info">
          ...
          <Item>
            ...
            <div slot="itemType">
              <el-input placeholder="中国" v-model="workObject.createCountry" :disabled="true"></el-input>
            </div>
          </Item>
          <Item>
      ...
      <div v-if="contentIndex === 2" style="width: 100%;">
        <finish-work>发起上传作品成功</finish-work>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    selectFile(e) {                                                                //上传文件事件
      if(document.getElementById("workFile").files.length!==0) {
        ...
        if (e.target.files[0].type.indexOf("image") > -1) {
          ...
          this.metadata.fileType = 4
        }else{
          this.$message({
            message: "文件类型只能是图片、视频、音频或文档"
          })
        }
      }
    },
    ...
    getCreateCity() {     //获得城市的编码
      let City = ""
      console.log(this.straightCity)
      City += this.createCity[1]
      let pos = this.straightCity.indexOf(this.createCity[0])
      if(pos !== -1) {
    ...
    City() {            //获得城市字符串
      ...
      let pos = this.straightCity.indexOf(this.createCity[0])
      if(pos !== -1) {
        city = CodeToText[this.straightCity[pos]]
      }
      console.log("输出的城市" + city)

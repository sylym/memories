<template>
  <div id="registerRights" style="width: 100%;min-width:1400px" v-loading="loading">
            ...
            <div slot="itemType" class="objItem">
              <div v-for="(item, index) in RegisterData.object.authors" :key="'author' + index" :style="{marginTop: index === 0 ? '' : '30px'}">
                ...
                <Item box-width="960px" box-height="auto">
                  ...
                  <div slot="itemType">
                    <tags :value-array="signStatus[index]" :tag-id="targetId + index" tag-width="91px" @getType="(pos)=>{getAuthorType(index, pos)}"></tags>
                  </div>
                </Item>
      ...
      <div id="uploadMaterial" v-show="status === 3"  style="margin-top: 180px;width: 100%;min-width:1400px;">
        <Item box-height="auto">
          ...
          <div slot="itemType" class="materialsBox">
            <div v-for="(item, index) in materials" :key="'material' + index" class="materialItem" :style="{marginTop: index === 0 ? '' : '15px'}" v-if="item.url!==''">
              <p class="fileName" style="color: red;width: 30px">{{item.isMust ? '*' : ''}}</p>
              <div class="materialLeft" v-if="item.url !== ''">
                <p class="fileName">{{ item.name }}</p>
                <a :href="item.url" :download="item.name" class="downloadBtn">{{item.url === "..." ? ' ' : '下载模板'}}</a>
              </div>
          ...
          <el-form class="SubjectForm">
            <div class="subjectItems">
              ...
              <div class="subjectItem">
                ...
                <div class="inputLeft">
                  <el-input v-model="country" disabled :popper-append-to-body="false"></el-input>
                </div>
              </div>
              <div class="subjectItem" v-if="accountType[nowPage] === '个人用户'">
                <div class="itemTips">
        ...
        <div v-show="nowStep === 3" class="rightsCollection">
          <div class="collectionItems">
            ...
            <div class="collItem" v-for="(obj, pos) in selectRights[nowPage]" :key="'rights'+nowPage + ' ' + pos">
              ...
              <input type="text" v-model="obj.explain" class="explainInput" placeholder="请填写权利说明" @input="(value) => {rightsExplain(nowPage, pos)}"></input>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      files: [   //true为必填 false为有条件时必填 null为无条件选填
        ...
        {type: "C4", name: "法人创作说明", url: "https://fxr-1312386734.cos.ap-beijing.myqcloud.com/%E7%99%BB%E8%AE%B0%E7%A1%AE%E6%9D%83%E6%9D%90%E6%96%99/%E6%B3%95%E4%BA%BA%E5%88%9B%E4%BD%9C%E6%9D%83%E5%88%A9%E5%BD%92%E5%B1%9E%E8%AF%B4%E6%98%8E%E4%B9%A6%EF%BC%885.31%EF%BC%89.docx", isMust: false},    //权利生产方式为法人时必填
        ...
        {type: "C13", name: "承受协议", url: "https://fxr-1312386734.cos.ap-beijing.myqcloud.com/%E7%99%BB%E8%AE%B0%E7%A1%AE%E6%9D%83%E6%9D%90%E6%96%99/%E6%9D%83%E5%88%A9%E4%B9%89%E5%8A%A1%E6%A6%82%E6%8B%AC%E6%80%A7%E8%BD%AC%E8%AE%A9%E5%90%88%E5%90%8C.docx", isMust: false},  //著作权取得方式为承受时
        {type: "C14", name: "赠予合同", url: "https://fxr-1312386734.cos.ap-beijing.myqcloud.com/%E7%99%BB%E8%AE%B0%E7%A1%AE%E6%9D%83%E6%9D%90%E6%96%99/%E8%91%97%E4%BD%9C%E6%9D%83%E8%B5%A0%E4%BA%88%E6%80%A7%E8%B4%A8%E8%BD%AC%E8%AE%A9%E5%8D%8F%E8%AE%AE%EF%BC%886.4%EF%BC%89%281%29.doc", isMust: false},  //著作权取得方式为赠予时
        {type: "C15", name: "...", url: "", isMust: null},
    ...
    await getWork().then(res=>{
      ...
      if(res.data.code === 200){
        ...
        for(let i = 0; i < res.data.data.length; i++){
          if(res.data.data[i].status === "版权注册成功" || res.data.data[i].status === "登记确权失败" || res.data.data[i].status === "登记确权草稿") {
  ...
  methods: {
      ...
      switch (this.status) {
        ...
        case 3: {
          ...
          for (let i = 0; i < this.materials.length; i++) {
            if (this.materials[i].isMust && this.materials[i].url !== "") {    // 如果是必填且this.materials[i].url不为空
              if (!this.materials[i].isUploaded) {
                nextFlag = 1
                break
              }
            }
          ...
          if (nextFlag === 0) {
            // 已经可以发起 删除多余材料
            let len = this.RegisterData.materials.length
            for(let i = 0; i < len; i++){
              for(let j = 0; j < this.RegisterData.materials.length; j++){
                if(this.RegisterData.materials[j].fileId.length === 0){
                  this.RegisterData.materials.splice(j, 1)
                  break
                }
              }
            }
            ...
            await launchRegister(this.RegisterData, this.headers).then(res=>{
              if (res.data.code === 200) {
                ...
                if(res.data.data === null){
                  that.loading = false
                  if(res.data.msg.indexOf("北京")!==-1){
                    that.$message({
                      message: res.data.msg,
                      type: "warning"
                    })
                  }else if(res.data.msg.indexOf("参数")!==-1){
                    that.$message({
                      message: res.data.msg,
                      type: "warning"
                    })
      ...
      if (works.length !== 0) {
        for(let i = 0 ;i < works.length; i++) {
          await getEnrollRes(works[i].workId, headers).then(res=>{             //两个id查找 通过作品名获得id再通过这个获得版权注册信息
            ...
            that.works.push(work)
            // if (res.data.data !== null) {
            //   if(res.data.data.status === "版权注册成功" || res.data.data.status === "登记确权失败" || res.data.data.status === "登记确权草稿") {
            //     console.log(res)
            //     that.searchWorks.push(res.data.data)                          //searchWorks保留原始返回数据  Works为处理过的获得的数据
            //     let work = new AddWork(0, works[i].workName, res.data.data.workId)   //通过作品名查找
      ...
      if (!isNaN(Number(content))) {
        await getEnrollRes(Number(content), headers).then(res=>{              //直接作为id 看看能否返回，如果有id(在版权注册能搜到)，则通过id获得作品信息中的名字
          if (res.data.data !== null || res.data.msg !== "作品不存在") {
            if(res.data.data.status === "版权注册成功" || res.data.data.status === "登记确权失败" || res.data.data[i].status === "登记确权草稿") {
          ...
          await getWorkById(Number(content), headers).then(res=>{   //返回作品信息
            if(res.data.data !== null){
              for(let i = 0; i < res.data.data.length; i++) {      //检查作品是否重复
                if(res.data.data[i].status === "版权注册成功" || res.data.data[i].status === "登记确权失败" || res.data.data[i].status === "登记确权草稿") {    //保证作品存证成功
                  let pos = 0
    ...
    async handleWork(item) {                                      //根据选择处理主体列表
        ...
        for(let j = 0; j < this.object.content.copyrights[i].userInfos.length; j++) {
          if (this.userInfosId.indexOf(this.object.content.copyrights[i].userInfos[j].userId) < 0) {        //如果这个用户还没存在用户列表里
            ...
            await getUserById(id, this.headers).then(res=>{
              console.log(res)
              if (res.data.data !== null) {
                let user = new AddUser(res.data.data.username, res.data.data.name !== undefined ? res.data.data.name : '', res.data.data.identificationNumber !== undefined ? res.data.data.identificationNumber : '', res.data.data.userId, res.data.data.username + ((res.data.data.name !== undefined) ? ("（**" + res.data.data.name + "）") : ''), res.data.data.type, "", "", "", "请完善主体信息")
                that.imgs.push([])
                ...
                that.birthday.push("")
                let material = new AddMaterial("","","0","","", "", "","", files)
      ...
      let flag2 = 0
      let flag3 = 0
      for (let i = 0; i < this.neccFilePos.length; i++) {   //无论如何必填的文件
      ...
      if (flag1 === 1 && flag2 === 1) {    //如果都获取到了
        ...
        for (let i = 0; i < this.unneFilePos.length; i++) {   // 非必要
    ...
    addUser() {     //添加主体用户
      ...
      let material = new AddMaterial("","","0","","", "", "","", [])
    ...
    async getUsers(value, index) {     //搜索用户信息
      ...
      await getUserInfoByName(value, headers).then(res=>{
        if (res.data.data !== null) {
          if(res.data.data.isRealName){
            let user = new AddSearchUser(res.data.data.username, (res.data.data.name !== undefined) ? res.data.data.name : '', res.data.data.name !== undefined ? (res.data.data.username + "（**" + res.data.data.name + "）") : res.data.data.username, res.data.data.userId, res.data.data.phone, res.data.data.type)
      ...
      if (!isNaN(Number(value))) {
        await getUserInfoByNumber(Number(value), headers).then(res=>{
          if (res.data.data !== null) {
            if(res.data.data.isRealName){
              let user = new AddSearchUser(res.data.data.username, (res.data.data.name !== undefined) ? res.data.data.name : '', (res.data.data.name !== undefined) ? (res.data.data.username + "（**" + res.data.data.name + "）") : res.data.data.username, res.data.data.userId, res.data.data.phone)
              that.searchUser[index].push(user)
            }
          }
        })
    ...
    async editNext(index) {
      ...
      switch (this.nowStep) {   //第一步
        ...
        case 2: {
          ...
          let Date1 = new Date(this.allUserInfos[index].dateStart)
          let Date2 = new Date(this.allUserInfos[index].dateEnd)
          let month1 = ""
          let month2 = ""
          let day1 = ""
          let day2 = ""
          month1 += (Date1.getMonth() < 9) ? "0" + (Date1.getMonth() + 1) : (Date1.getMonth() + 1)
          day1 += (Date1.getDate() < 10) ? "0" + Date1.getDate() : Date1.getDate()
          month2 += (Date2.getMonth() < 9) ? "0" + (Date2.getMonth() + 1) : (Date2.getMonth() + 1)
          ...
          this.RegisterData.subjects[index].material.dateStart = Date1.getFullYear() + "-" + month1 + "-" + day1
          this.RegisterData.subjects[index].material.dateEnd = Date2.getFullYear() + "-" + month2 + "-" + day2
          ...
          if (this.accountType[index] === "机构") {
            ...
            let Date3 = new Date(this.allUserInfos[index].birthday)
            let month3 = ""
            let day3 = ""
            month3 += (Date3.getMonth() < 9) ? "0" + (Date3.getMonth() + 1) : (Date3.getMonth() + 1)
            day3 += (Date3.getDate() < 10) ? "0" + Date3.getDate() : Date3.getDate()
            this.RegisterData.subjects[index].material.birthday = Date3.getFullYear() + "-" + month3 + "-" + day3
            if (
    ...
    editComplete(index) {     //完成编辑主体信息
      let flag = false  // 如果是有人身权的 必填
      if(this.nowPage < this.userInfos.length){
        for(let i = 0; i < this.selectRights[index].length; i++){
          if(this.selectRights[index][i].isChoose){
            flag = true
            break
          }
        }
      }else flag = true
      if(flag){  // 如果是人身权且已经至少选了一项权利或者没有人身权
        this.displayImg1.pop()
        this.displayImg2.pop()
        if (this.displayImg3.length !== 0) {
          this.displayImg3.pop()
        }
        if (this.nowPage < this.userInfos.length) {  // 如果是有人身权的 可以编辑完成
      ...
      if (this.selectRights[index][pos].isChoose) {   //已选变未选
        ...
        for (let i = 0; i < this.RegisterData.subjects[index].rights.length; i++) {
          if (this.RegisterData.subjects[index].rights[i].category === this.selectRights[index][pos].value) {
            console.log("已选" + this.selectRights[index][pos].value + "变未选")
    ...
    async searchOther(value, index) {                      //搜索无财产权的用户
      let that = this
      ...
      await getUserInfoByName(value, this.headers).then(res=>{
        if (res.data.data !== null) {
          ...
          for (let i = 0; i < that.userInfos.length; i++) {
            if (that.userInfos[i].username === res.data.data.username && i !== index) {
              flag = 1
              break
            }
          }
          if (flag === 0) {
            that.otherSearchUsers[index].push({type: res.data.data.type, username: res.data.data.username, name: (res.data.data.username !== undefined) ? res.data.data.username : '', userId: res.data.data.userId, nameString: (res.data.data.name !== undefined) ? (res.data.data.username + "（**" + res.data.data.name + "）") : ''})
          }
        }
      })
    ...
    async uploadFile(e,index) {
      if (e.target !== null) {
        ...
        forms.append("workId", this.RegisterData.workId)
        forms.append("type", this.materials[index].type)
        await upload(forms, this.headers).then(res=>{
          // let file = {fileId: res.data.data.fileId, fileType: "1"}**************
          // let flag = 0
          if (res.data.data !== null) {
    ...
    deleteFile(index) {
      ...
      this.materials[index].uploadFileUrl = ""
      this.materials[index].uploadFileName = ""

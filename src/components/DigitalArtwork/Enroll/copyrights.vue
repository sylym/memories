<template>
  <div id="Copyrights" v-loading="loading">
    <!--进度条-->
    ...
    <div id="copy-unit" v-show="status === 1">
      <Item margin-top="23px" id="input">
        ...
        <el-select v-model="workTitle" placeholder="请输入作品名称/平台编号进行搜索" filterable slot="itemType" :popper-append-to-body="false" :remote-method="getWorks" remote @change="displayAuthor">
            ...
            <div id="copyrights">
              <div id="copyRight" class="addUser" v-for="(unit,pos) in workUnit" :key="'unit' + pos" :style="{marginTop: pos===0?'0':'30px'}">
                ...
                <div class="titleInfo" id="search">
                  <div>
                    <el-select v-model="unit.searchContent" placeholder="请输入用户名/手机号进行搜索" :remote-method="(value=>{searchUser(value, pos)})" filterable :popper-append-to-body="false" remote disabled>
...
function UserInfos(userId, copyrightExplain, proportion) {   //userInfos列表
  this.userId = userId
  this.copyrightExplain = copyrightExplain
  if(proportion!==""){
    this.proportion = proportion + ".0%"
  }else{
    this.proportion = "NA"
  }
}
...
export default {
  ...
  data() {
    return {
      tempId: 0,     // 作品id 用于检查上次选择的作品和再次选择的作品是否相同
      Status: 0,
      Info: 0,
      status: 1,
  ...
  methods: {
    ...
    async searchUser(value, pos) {                               //搜索用户信息
      ...
      await getUserInfoByName(value, this.headers).then((res)=>{   //添加到搜索列表    后期还要修改    搜索搜索
        if (res.data.data !== null) {
          let flag = 0
          for (let i = 0; i < that.workUnit.length; i++) {               //检查是否重复，已经选择过的不能再选择  还有检查是否已实名
            if (i !== pos && that.workUnit[i].searchContent !== "") {
              if (that.workUnit[i].replyResult[that.workUnit[i].searchContent].userId === res.data.data.userId || res.data.data.name === undefined || res.data.data.name === null) {
                flag = 1
                break
              }
            }
          ...
          if (flag === 0) {
            that.workUnit[pos].replyResult.push({userInfo: res.data.data.username + (res.data.data.name !== undefined ? ("（**" + res.data.data.name + "）") : ''), userId: res.data.data.userId, username: res.data.data.username})
          }
        }
      })
    ...
    createCopyright(data,pos,index) {      //创建版权单元对象    保存草稿时用
      ...
      this.Authorization.copyrights.push(copyrights)  // 添加版权信息
    ...
    addCopyright(data,pos,index) {        //添加版权单元对象     保存草稿or下一步时
      ...
      // console.log(this.copyUnit[index].name)
      let flagOuter = true                     //外边是否已存在
      // that.workUnit[i].copyUnit[j].select
    ...
    displayProportion(pos,index) {                                     //修改比例显示状态
      if(this.workUnit[pos].copyUnit[index].select) {                  //是否已经可选
        ...
        let nowProp = Number(this.workUnit[pos].copyUnit[index].proportion)
        if(isNaN(nowProp)){  // 如果参数错误
          this.workUnit[pos].copyUnit[index].proportion = ""
          this.$message({
            message: "输入的数据必须为整数",
            type: "warning"
          })
        ...
        for(let i = 0; i < this.workUnit.length; i++) {
          if(this.workUnit[i].copyUnit[index].select) {  // 如果已经被选择 可以填写
            let prop = Number(this.workUnit[i].copyUnit[index].proportion)  // 比例
            if(this.workUnit[i].copyUnit[index].proportion !== "" && isNaN(prop) === false) { // 如果数字不为空
              if (prop <= this.redundantPro[index]) {  // 如果比例正确
                isRedundant = true
                ...
                this.workUnit[i].copyUnit[index].proportion = ""
              }
            }
          }
        }
    ...
    async keepDraft() {                      //保存草稿
      ...
      if (this.status === 1) {
        for(let i = 0; i < this.Authorization.copyrights.length; i++){
          this.Authorization.copyrights[i].userInfos = []
        }
        for(let i = 0; i < this.workUnit.length; i++) {
          // console.log(i)
          if(this.workUnit[i].searchContent !== ""){
            let username = this.workUnit[i].replyResult[this.workUnit[i].searchContent].username   //用户名
            await getUserInfoByName(username, this.headers).then(function (res){  // 获取用户信息
              //console.log(res)
              if (res.data.data !== null) {
                for(let j = 0; j < 13; j++) {
                  // console.log(that.workUnit[i].copyUnit[j].name + " " + that.workUnit[i].copyUnit[j].select)
        ...
        let propFlag = true
        let propFlag2 = false
        for(let i = 0; i < this.redundantPro.length; i++){
          if(this.redundantPro[i] !== 100 && this.redundantPro[i] !== 0){
            // 如果比例不是100或0说明数据不符合
            propFlag = false
            break
          }
          if(this.redundantPro[i] === 0){
            // 如果存在为0的，那么应该全部为0
            propFlag2 = true
          }
          if(propFlag2&&this.redundantPro[i] !== 0){
            propFlag = false
            break
          }
        }
        ...
        if(this.workTitle !== 0 && this.workUnit.length > 0 && OuterFlag && propFlag) {
          ...
          if(!propFlag){
            this.$message({
              message: "权利分配比例只允许不填或每项权利总和为100，并且每项权利至少有一人选",
              type: "warning"
            })
          }else {
            this.$message({
        ...
        if(this.publish.status === 1) {
          ...
          if(this.publish.city !== 0 && this.publish.date !== 0) {  //检查是否都已填
            pFlag = true
            console.log(pFlag)
          }
        }else {
        ...
        if(this.comeout.status === 1) {
          ...
          if(this.comeout.city !== 0 && this.comeout.date !== 0) {  //检查是否都已填
            cFlag = true
          }
        }else {
        ...
        if(this.issue.status === 1) {
          ...
          if(this.issue.city !== 0 && this.issue.date !== 0) {  //检查是否都已填
            iFlag = true
          }
        }else {
    ...
    getPublishCity() {
      ...
      // }
      let city = ""
      let pos = this.straightCity.indexOf(this.Publish.city[0])
      if(pos !== -1) {
        city = this.straightCity[pos]
      }else {
        city = this.Publish.city[1]
      }
      return Number(city)
    },
    getComeOutCity() {
      ...
      // }
      let city = ""
      let pos = this.straightCity.indexOf(this.comeOut.city[0])
      if(pos !== -1) {
        city = this.straightCity[pos]
      }else {
        city = this.comeOut.city[1]
      }
      return Number(city)
    },
    getIssueCity() {
      ...
      // }
      let city = ""
      let pos = this.straightCity.indexOf(this.Issue.city[0])
      if(pos !== -1) {
        city = this.straightCity[pos]
      }else {
        city = this.Issue.city[1]
      }
      return Number(city)
    },
    getPublishTime() {
      ...
      await getWorkByName(value, this.headers).then(res=>{
        if (res.data.data !== null) {
          for (let i = 0; i < res.data.data.length; i++) {
            if (res.data.data[i].status === "作品存证成功" || res.data.data[i].status === "版权注册失败" || res.data.data[i].status === "版权注册草稿") {
    ...
    async displayAuthor(value){   // 展示信息
      if(this.tempId === this.workTitle){
        // 如果相等 不作处理
      }else {
        this.tempId = this.workTitle
        this.Authorization.workId = this.tempId
        for(let i = 0; i < this.Authorization.copyrights.length; i++){
          this.Authorization.copyrights[i].userInfos = []
        }
        // 否则 清空所有数据
        // this.Authorization = {
        //   workId: this.tempId,                     //作品id
        //       copyrightGetType: 0,           //权利取得方式 0-原创 1-转让 2-继承
        //       //草稿分界***********************
        ...
        //   ]
        // }
        while(this.workUnit.length!==0){  // 清空版权单元
          this.workUnit.pop()
        }
        this.redundantPro = [100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100]
        // 初始化一遍后添加数据
        let authors = []
        let that = this
        await getWorkInfo(this.tempId, this.headers).then(res=>{
          if(res.data.code===200){
            console.log(res.data)
            authors = res.data.data.deposit.content.depositAuthorVOS   // 获取到的作者信息
          }
        })
        for(let i = 0; i < authors.length; i++){
          this.addUser()
          await getUserInfoByName(authors[i].username, this.headers).then(res=>{
            if(res.data.code === 200){
              console.log(res.data)
              that.workUnit[i].replyResult.push({userInfo: res.data.data.username + (res.data.data.name !== undefined ? ("（**" + res.data.data.name + "）") : ''), userId: res.data.data.userId, username: res.data.data.username})
              that.workUnit[i].searchContent = 0
            }
          })
        }
      }

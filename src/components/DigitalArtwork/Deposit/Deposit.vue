<template>
  <div style="width: 100%;min-width: 1400px;position: absolute;display: flex;flex-direction: column;align-items: center;" id="deposit" v-loading="loading">
...
import { getWorkLists, getUserInfoByName, getUserInfoByNumber, launchDeposit, updateDeposit, getWorkByName, getWorkById, getDepositInfo, getWork } from '@/api.js'
...
export default {
  ...
  methods: {
    async getWorkOptions() {    //获得作品的列表
      try {
        ...
        v.data.data.forEach(item => {   //遍历
          if(item.status === "作品存证草稿" || item.status === "作品存证失败") {
            that.works.push({
              workId: item.workId,
    ...
    async getWork(content) {                     //实时查找作品
      ...
      try {
        ...
        v2.data.data.forEach(item => {   //遍历
          ...
          for (let i = 0; i < works.length; i++) {
            if (Number(works[i].workId) === item.workId) {
              flag = 1
            }
          }
          if (flag === 0) {                      //检查是否重复
      ...
      for(let i = 0; i < works.length; i++) {
        await getDepositInfo(works[i].workId, this.headers).then(res => {
          if (res.data.data === null) {   //null说明还没存证过
            that.works.push(works[i])
          }else{
            if(res.data.data.status === "作品存证失败" || res.data.data.status === "作品存证草稿") {
              that.works.push(works[i])
            }
          }
        })
    ...
    queryWork() {
      ...
      if(this.works[this.workName].createCountry === "中国" || this.works[this.workName].createCountry === "0") {
        this.dataContent.content.createCountry = "0"
      }else {
        this.dataContent.content.createCountry = "1"
      }
      ...
      console.log(this.works[this.workName])
      ...
      this.editForm.createCountry = "中国"
      //创作国家
      this.Info.createCountry = this.dataContent.content.createCountry
      // if(this.dataContent.content.createCountry === "0") {
      //   this.Info.createCountry = "中国"
      // }
      //创作时间
      ...
      if (!isNaN(phone)) {
        await getUserInfoByNumber(value, this.headers).then(res=>{           //返回的是数组  通过手机号查找
          if (res.data.data!==null) {
            ...
            if(res.data.data.isRealName){   // 检查用户是否实名
      ...
      await getUserInfoByName(value, this.headers).then(res=>{             //通过用户名查找
        ...
        if (res.data.data !== null) {
          ...
          for(let j = 0; j < that.authors[index].result.length; j++) {
            if (that.authors[index].result[j].userId ===  res.data.data.userId || !res.data.data.isRealName) {   //检查id是否重复 和 用户是否实名
              flag = 1    //如果出现了用户信息重复
            }
          }
          if (flag === 0) {
    ...
    change(index) {    //修改id即可
      ...
      let authorInfosText = ""
      console.log(this.authors[index].result[this.authors[index].search].name)
      if (this.authors[index].result[this.authors[index].search].name === null || this.authors[index].result[this.authors[index].search].name === undefined) {   //是否有实名认证信息
        authorInfosText = this.authors[index].result[this.authors[index].search].username
      } else {
        authorInfosText = this.authors[index].result[this.authors[index].search].username + '（**' + this.authors[index].result[this.authors[index].search].name + '）' + '**************' + this.authors[index].result[this.authors[index].search].identificationNumber //获得字符串
      }
      this.authorInfosText[index] = authorInfosText
